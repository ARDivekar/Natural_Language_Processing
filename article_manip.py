		...
		try:
			...
			try:
				...
				try:
					...
					print "\nTried 3 times. Cannot access url:\n%s. \nHence, cannot make HTML_page_Obj\n"%page_url
		if response != None:
...
class ArticleObject:
	...
	def print_details(self):
		print "\n\n\nURL:\t%s"%(self.article_url)
		print "\nHEADLINE:\t%s"%(self.article_headline)
		print "\nDATE:\t%s"%(self.article_date)
		print "TIME:\t%s"%(self.article_time)
		for article_alt_headline in self.article_alt_headline_list:
			print "\nALT HEADLINE:\t%s"%(article_alt_headline)
		print "\n\nARTICLE TEXT:\n%s"%(self.article_text)
