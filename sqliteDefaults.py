...
def build_insert_query(tablename, insert_params_list, tuple_values_list):
	...
	insert_query+=") VALUES "
	#print insert_query
	if tuple_values_list[0][0] is not None:
		insert_query+="\n(%s"%escape_value(tuple_values_list[0][0])
	else: insert_query+="\n(NULL"
	for j in range(1,len(tuple_values_list[0])):
		if tuple_values_list[0][j] is not None:
			insert_query+=" ,%s"%escape_value(tuple_values_list[0][j])
		else: insert_query+=" , NULL"
	insert_query+=")"


	for i in range(1,len(tuple_values_list)):
		insert_query+=",\n(%s"%escape_value(tuple_values_list[i][0])
		for j in range(1,len(tuple_values_list[i])):
			if tuple_values_list[i][j] is not None:
				insert_query+=" ,%s"%escape_value(tuple_values_list[i][j])
			else: insert_query+=" , NULL"
		insert_query+=")"
	insert_query+=";"
	# print insert_query
